...
<script>
    ...
    $(function () {
       $("a[orderStatus]").click(function () {
           ...
           if(orderStatus=='all') $("table[orderStatus]").show();
           else{
       ...
       $("#deleteConfirmModal").on("hidden.bs.modal",function (e) {
           if(deleteOrder){
               ...
               $.post(page,{"oid":deleteOrderid},function (result) {
                   if(result=="success") $("table.orderListItemTable[oid="+deleteOrderid+"]").hide();
        ...
        <c:forEach items="${os}" var="o">
            <table class="orderListItemTable" orderStatus="${o.status}" oid="${o.id}">
                <tr class="orderListItemFirstTR">
                    ...
                    <td colspan="1">
                        <a class="wangwanglink" href="#nowhere"><div class="orderItemWangWangGif"></div></a>
                    </td>
                    <td class="orderItemDeleteTD">
                        <a class="deleteOrderLink" oid="${o.id}" href="#nowhere"><span  class="orderListItemDelete glyphicon glyphicon-trash"></span></a>
                    </td>
                </tr>
                        ...
                        <c:if test="${st.count==1}">
                            ...
                            <td valign="top" rowspan="${fn:length(o.orderItems)}" class="orderListItemButtonTD orderItemOrderInfoPartTD" width="100px">
                                <c:if test="${o.status=='waitConfirm' }">
                                    <a href="foreconfirmPay?oid=${o.id}"><button class="orderListItemConfirm">确认收货</button></a>
                                </c:if>
                                <c:if test="${o.status=='waitPay' }">
                                    <a href="alipay.jsp?oid=${o.id}&total=${o.total}"><button class="orderListItemConfirm">付款</button></a>
                                </c:if>
                                ...
                                <c:if test="${o.status=='waitReview' }">
                                    <a href="forereview?oid=${o.id}"><button  class="orderListItemReview">评价</button></a>
                                </c:if>
                            </td>
                        </c:if>
                    </tr>
